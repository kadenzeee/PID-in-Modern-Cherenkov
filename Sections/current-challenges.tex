...
Much of modern detector research is placed into improving photosensor performance in high-rate collider environments. 
Along with rapidly developing SiPMs, improvements are also being made to MCP-PMTs, both towards better spatial and temporal resolution. A process known as 
atomic layer deposition (ALD) coats the MCPs with resistive layers to precisely tune the performance;\nolinebreak\cite{ald-mcps-Popecki_2016} this has been shown to significantly improve sensor ageing.\nolinebreak\cite{2013NIMPA.732..388C} Research into Large Area Picosecond Photodetectors (LAPPDs)
...
reduce dead time and manage large data volumes while maintaining low noise and power efficient performance. Close integration of sensors with dedicated readout 
...
to significantly reduce the price tag on detector arrays. Reuse of retired detector parts is also a potential cost saving measure; the recent transport of legacy BaBar boxes for use in the hpDIRC at Brookhaven is one example of successful reuse.\nolinebreak\cite{hpDIRC-Greg_2025}
