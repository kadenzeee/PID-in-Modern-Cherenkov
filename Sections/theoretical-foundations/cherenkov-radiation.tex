...
the ratio between the speed of light \(c\) and the phase velocity \(v_p\). The condition for Cherenkov radiation is then \(v > c/n\).

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{Images/cherenkov-diagram-pdg.png}
...
\end{equation}
for a particle with charge \(ze\) and wavelength \(\lambda\), moving through a medium with refractive index \(n(\lambda)\), and where \(\alpha\) is the fine structure constant.\nolinebreak\cite{Frank_1937}
For most uses, Equ \nolinebreak\ref{equ:frank-tamm-formula} 
should be integrated over the region \(\beta n(\lambda) > 1\). The number of photons \(N\) emitted per unit length is inversely proportional to the square of the wavelength, explaining the characteristic blue glow of Cherenkov radiation.
