...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{Images/pmt.png}\caption{\textbf{Design of traditional PMT.} Photons incident on a cathode emit a photoelectron. The current is amplified through a series of dynodes at high voltage. The resulting current is recorded on an anode plate.\label{fig:traditional-pmt}}
\end{figure}

Modern photocathodes used in Cherenkov imaging applications typically have a quantum efficiency (QE) of up to ~30\%, meaning that only 30\% of incident photons satisfy the work function and produce a photoelectron that reaches the dynodes. \cite{average-QE-pmts_2018} The QE of a photocathode is wavelength dependent. For applications in imaging Cherenkov photons, a peak QE around the blue wavelengths is most desirable.
...
PMTs are versatile devices that are still employed in many applications, including Cherenkov detectors. However, due to the distance between dynodes, they are sensitive to magnetic fields. Since many detector systems at research facilities employ the use of solenoids, PMTs are often unfeasible. The solution is to significantly restrict the space between the dynodes.